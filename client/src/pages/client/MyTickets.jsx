...
export default function MyTickets() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const { items, pagination } = await getMyTickets({
          pageNumber: page,
          pageSize: 10,
    ...
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ padding: '24px', backgroundColor: colors.bgPage, minHeight: '100vh' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <input
          ...
          placeholder={t('search for a specific ticket') || 'Search by title or description...'}
          style={{
            width: '100%',
            maxWidth: 420,
            padding: '10px 14px',
                ...
                {items.length === 0 ? (
                  ...
                  items.map((tk, idx) => (
                    ...
                    >
                      ...
                      <td style={{ padding: '14px 24px' }}>
                        <button
                          onClick={() => navigate(`/client/tickets/${tk.id}`)}
                          style={{
                            padding: '6px 16px',
                            borderRadius: 8,
                            border: 'none',
